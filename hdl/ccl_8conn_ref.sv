...
module tpss_ccl_8conn #(
    ...
    // --- BRAM DEFINITIONS ---
    parameter MEM_DEPTH = WIDTH * HEIGHT;    // Total number of pixels (frame size)
    parameter ADDR_BITS = $clog2(MEM_DEPTH); // Required bits for addressing all pixels
    parameter ENTRY_WIDTH = LABEL_BITS + GEN_BITS;
    parameter X_BITS = $clog2(WIDTH);
    parameter Y_BITS = $clog2(HEIGHT);
