...
module top_level_4blob
    ...
    generate
        if (!STUB_HEAVY) begin : gen_bev_path
            ...
            cle_module_8conn #(
                ...
                .HEIGHT(BEV_VRES), 
                .LABEL_BITS(10),
                .MAX_COMPONENTS(16) // Increased to support 4 blobs comfortably with margin
            ) bev_ccl (
                .clk(clk_pixel),
                .rst(sys_rst_pixel),
    ...
    always_ff @(posedge clk_pixel) begin
        if (sys_rst_pixel) begin
            ...
            valid_midpoints_hold <= valid_midpoints;
            left_midpoint_hold   <= left_midpoint;
            right_midpoint_hold  <= right_midpoint;
            total_midpoint_hold  <= total_midpoint;
        end
    end

    // Use stretched/held values if present.
    wire        valid_midpoints_eff = valid_midpoints | valid_midpoints_hold;
    wire [10:0] left_midpoint_eff   = valid_midpoints_hold ? left_midpoint_hold  : left_midpoint;
    wire [10:0] right_midpoint_eff  = valid_midpoints_hold ? right_midpoint_hold : right_midpoint;
    wire [10:0] total_midpoint_eff  = valid_midpoints_hold ? total_midpoint_hold : total_midpoint;
    ...
    lane_annotation lane_overlay (
        ...
        .rst(sys_rst_pixel),
        .lanes_valid(lanes_valid),
        .lane_left_x(lane_left_screen_x),
        // .lane_left_y(lane_left_screen_y),
        .lane_right_x(lane_right_screen_x),
    ...
    assign led[10] = lane_valid_q[0];
    assign led[11] = lane_valid_q[1];
    assign led[12] = 1'b0; // lane_valid_q[2] removed
    assign led[13] = 1'b0; // lane_valid_q[3] removed
